...
def calculate_variance(comun_id):
    cursor.execute("SELECT betweenness, closeness, eigenvector FROM user_to_user_centrality WHERE comun_id = %s", (comun_id,))
...
cursor.execute("SELECT DISTINCT comun_id FROM user_to_user_centrality")
...
for comun_id in comun_ids:
    ...
    if eigenvector_varianc > 0.016481569437021115:
        print(f"comun_id = {comun_id} eigenvector_varianc = {eigenvector_varianc}")
